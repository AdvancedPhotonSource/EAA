...
def test_merge_tools_ignores_hidden_tool_name_clashes():
    ...
    task_manager.register_tools([FirstTool(), SecondTool(), DuplicateSecondTool()])

    assert [type(tool) for tool in task_manager.tool_manager] == [FirstTool, SecondTool]
    assert task_manager._collect_tool_names(task_manager.tool_manager) == {"first", "second"}


def test_task_manager_owns_tool_manager_not_tools():
    task_manager = BaseTaskManager(build=False, checkpoint_db_path=None)
    ...
    assert not hasattr(task_manager, "bubblewrap_visible_dirs")
...
def test_tool_manager_disable_workspace_tool_unregisters_workspace_specs():
    task_manager = BaseTaskManager(checkpoint_db_path=None)
    manager = task_manager.tool_manager

    manager.disable_workspace_tool()

    assert manager.file_system_tool is not None
    assert manager.image_rendering_tool is not None
    assert manager.uv_tool is not None
    assert manager.file_system_tool not in manager
    assert manager.image_rendering_tool not in manager
    assert manager.uv_tool not in manager
    assert "file_system_tool.read_file" not in task_manager.tool_executor.tool_specs
    assert "image_rendering_tool.render_image_for_agent" not in task_manager.tool_executor.tool_specs
    assert "uv_tool.uv" not in task_manager.tool_executor.tool_specs


def test_tool_manager_disable_coding_tool_unregisters_coding_specs():
    task_manager = BaseTaskManager(checkpoint_db_path=None)
    manager = task_manager.tool_manager

    manager.disable_coding_tool()

    assert manager.python_coding_tool is not None
    assert manager.bash_coding_tool is not None
    assert manager.python_coding_tool not in manager
    assert manager.bash_coding_tool not in manager
    assert "python_coding_tool.execute" not in task_manager.tool_executor.tool_specs
    assert "bash_coding_tool.execute" not in task_manager.tool_executor.tool_specs


def test_tool_manager_controls_coding_tools_collectively_and_individually():
    task_manager = BaseTaskManager(checkpoint_db_path=None)
    manager = task_manager.tool_manager

    manager.set_coding_tool_request_approval(False)

    assert manager.python_coding_tool.require_approval is False
    assert manager.bash_coding_tool.require_approval is False
    assert task_manager.tool_executor.tool_specs["python_coding_tool.execute"].require_approval is False
    assert task_manager.tool_executor.tool_specs["bash_coding_tool.execute"].require_approval is False

    manager.set_python_coding_tool_request_approval(True)

    assert manager.python_coding_tool.require_approval is True
    assert manager.bash_coding_tool.require_approval is False
    assert task_manager.tool_executor.tool_specs["python_coding_tool.execute"].require_approval is True
    assert task_manager.tool_executor.tool_specs["bash_coding_tool.execute"].require_approval is False

    manager.set_coding_tool_sandbox_type("bubblewrap", visible_dirs=["/tmp"])

    assert manager.python_coding_tool.sandbox_type == "bubblewrap"
    assert manager.bash_coding_tool.sandbox_type == "bubblewrap"
    assert manager.python_coding_tool.bubblewrap_visible_dirs == ["/tmp"]
    assert manager.bash_coding_tool.bubblewrap_visible_dirs == ["/tmp"]

    manager.set_bash_coding_tool_sandbox_type(None)

    assert manager.python_coding_tool.sandbox_type == "bubblewrap"
    assert manager.bash_coding_tool.sandbox_type is None
...
def test_subagent_manager_omits_subagent_tool_and_appends_prompt():
    ...
    assert task_manager.tool_manager.subagent_tool is None
...
def test_launch_subagent_inherits_tools_except_subagent(monkeypatch, tmp_path):
    ...
    parent.register_tools([EchoTool(), subagent_tool])
    captured = {}
