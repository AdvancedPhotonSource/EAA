...
def process_xrfdata(exe_path, parent_dir, scan_num_mda,
                    det_range = "0:0",
    ...
    if scan_num_mda in mda_files:
        args = ["--dir", parent_dir,
                "--files", scan_num_mda,
                "--detector-range", det_range,
                "--export-csv", "",
                "--quantify-with", quantify_with,
                "--fit", fitting_type,
                "--optimizer-num-iter", str(num_iter),
                "--generate-avg-h5", ""]
...
def load_h5(img_h5_path, fit_type = ["NNLS", "ROI"], fsizelim = 1e3):
    """
...
def save_xrfdata(img_h5_path, output_dir, cmap = 'inferno', 
                 elms = None, vmax_th = 99, vmin = 0):
    """
    ...
    if data:
        ...
        for e in plot_elms:
            ...
            fig = plot_xrfdata(plotarr, xaxis, yaxis, data['scan'], 
                         e, cmap, vmax, vmin)
