...
Creates a feature tracking task manager that acquires and registers images to locate a target feature.
...
## `FeatureTrackingTaskManager.run_fov_search`
```python
def run_fov_search(
    self,
...
## `FeatureTrackingTaskManager.run_feature_tracking`
```python
